...
const HeaderHome = ({ regions }: props) => {
    ...
    return (
        <nav className='flex items-center  box-border justify-between container mx-auto h-[140px] px-12'>
            <section className='border-2 border-whiteDMTLME flex  justify-between items-center shadow-md w-[580px] p-1 rounded-md h-[60px]'>
            ...
            <section className='relative border-2 border-whiteDMTLME shadow-md  w-[300px] p-2 rounded-md h-[60px]'>
                <section className='box-border h-full' onClick={() => handleShow()}>
                    <section className='flex items-center justify-between h-full'>
                        <span className='ml-4 text-lg font-bold capitalize'>{nameOptions || "Selected your Region"}</span>
                        {
                            isShowOptions ?
